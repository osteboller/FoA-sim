...
// Global state for the entire Arena feature
...
let arenaCategory = 'warriors'; // 'warriors', 'pps', 'weapons', 'pods'
let autoBattleTimer = null;
let currentArenaView = 'builder'; // 'levels', 'builder', 'rules'
let selectedLevel = 1;
...
function startBattle() {
    ...
    // Initialize Weapon Indicator in Top Bar
...
function activateWeapon() {
    ...
    if(currentAlien && weapon) {
        // Check match: Neutralizer (weapon) OR Metallic OR Exact type match OR hybrid contains type
        const match = (weapon.type === 'weapon') || // Neutralizer virker for alle
                      (currentAlien.type === 'metallic') || // RAMMs kan bruge alt
                      (currentAlien.type === weapon.type) || 
                      (currentAlien.type === 'hybrid' && (currentAlien.c1 === weapon.type || currentAlien.c2 === weapon.type));
        
        if(match) {
            if (weapon.type === 'weapon') { // Neutralizer
            ...
            // Visuelt indiker at våben er brugt
            const wInd = document.getElementById('battle-weapon-indicator');
            if(wInd) wInd.style.opacity = '0.3';

            resolveRound(); // Gå direkte til afgørelse
        } else {
...
function playNextRound(manualIndex = -1) {
    // Stop hvis kampen er slut
    if(battleState.playerScore >= 4 || battleState.enemyScore >= 4 || battleState.round >= 7) return;

    // Hvis vi allerede er i matchup fasen (og har set animationerne), går vi til afgørelse
    if (battleState.phase === 'matchup') {
        resolveRound();
        return;
    }

    // --- FASE 1: DET FØRSTE CLASH ---
    ...
    if(autoBattleTimer) clearInterval(autoBattleTimer); // Stop timer while animating
    
    renderBattleField(); // Viser kortene (fjendens power er skjult)
    ...
    setTimeout(() => {
        ...
        if (!isPowerDuel) {
            ...
            setTimeout(() => {
                // Gå direkte til state-opdatering, da animationen allerede er startet
                _finalizeRound(winner, currentAlien, currentEnemy);
            }, 1600);
        } else {
            // SCENARIE B: POWER DUEL
            showAnnouncement("<span style='color:var(--gold); text-shadow: 0 0 20px var(--gold);'>POWER DUEL!</span>", null, 1500);
            
            const pIndex = battleState.playerMoves[battleState.round];
            const pFigure = document.getElementById(`player-figure-${pIndex}`);
            const eFigure = document.getElementById('battle-enemy-active').firstElementChild;
            if(pFigure) { pFigure.classList.remove('anim-fly-in'); pFigure.classList.add('anim-shake'); }
            if(eFigure) { eFigure.classList.remove('anim-fly-in-enemy'); eFigure.classList.add('anim-shake'); }
            ...
            if (battleState.auto) {
                setTimeout(() => resolveRound(), 1600);
            } else if (weaponUsable) {
                setTimeout(() => updateBattleControls(), 1600);
            } else {
                setTimeout(() => resolveRound(), 1600);
            }
        }
...
function resolveRound() {
    ...
    // Genopfrisk feltet (fjerner gamle animationer som shake)
    renderBattleField(battleState.round);
    
    // FIX: Fjern fly-in animationen med det samme, så den ikke genstarter eller blokerer
    ...
    // 1. POWER REVEAL (Kun ved Power Duel)
    if (isPowerDuel) {
        const eFigure = document.getElementById('battle-enemy-active').firstElementChild;
        if(eFigure) {
            const label = eFigure.querySelector('.figure-label');
            if(label) {
                label.innerText = `${eFig.name} (${eFig.power + (eFig.type === 'hybrid' ? 3 : 0)})`;
                label.classList.add('anim-power-reveal');
            }
        }
    }
    ...
    setTimeout(() => {
        ...
        // UNIVERSAL VINDER/TABER ANIMATION
        // 4. TABER-ANIMATION
...
function _finalizeRound(winner, pFig, eFig) {
    ...
    // Skjul fjendens power i loggen hvis den var irrelevant
    let ePowerText = ePower;
    if (!isPowerRelevant(pFig, eFig)) ePowerText = "???";
...
function endMatch() {
    ...
    if(battleState.playerScore > battleState.enemyScore) {
        ...
        if (selectedLevel === (state.maxLevel || 1)) {
            ...
            state.arenaLevel = state.maxLevel; // Legacy support
    ...
    showMatchResultOverlay(
        isWin,
        () => { currentArenaView = 'builder'; initArena(); save(); }, // Saml Hold
        () => { currentArenaView = 'levels'; initArena(); save(); },  // Kampagne
        () => { startBattle(); }, // Prøv Igen
        () => { selectLevel(selectedLevel + 1); } // Næste Modstander
    );
}
...
function selectLevel(lvl) {
    if (lvl > (state.maxLevel || 1)) return; // Låst
    selectedLevel = lvl;
    startBattle();
}
